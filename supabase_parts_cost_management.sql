...
create table if not exists public.part_cost_unpaid_carryovers (
  id bigserial primary key,
  unpaid_year integer not null,
  supplier_name text not null,
  amount numeric not null default 0,
  memo text,
...
create index if not exists part_cost_unpaid_carryovers_paid_idx
on public.part_cost_unpaid_carryovers (is_paid, unpaid_year, supplier_name);
...
drop trigger if exists part_cost_unpaid_carryovers_set_updated_at
on public.part_cost_unpaid_carryovers;

create trigger part_cost_unpaid_carryovers_set_updated_at
before update on public.part_cost_unpaid_carryovers
for each row
execute function public.set_part_cost_updated_at();
...
alter table if exists public.part_cost_unpaid_carryovers enable row level security;
...
drop policy if exists part_cost_unpaid_carryovers_authenticated_read
on public.part_cost_unpaid_carryovers;

create policy part_cost_unpaid_carryovers_authenticated_read
on public.part_cost_unpaid_carryovers
for select
to authenticated
using (public.current_app_user_id() is not null);

drop policy if exists part_cost_unpaid_carryovers_authenticated_write
on public.part_cost_unpaid_carryovers;

create policy part_cost_unpaid_carryovers_authenticated_write
on public.part_cost_unpaid_carryovers
for all
to authenticated
using (public.current_app_user_id() is not null)
